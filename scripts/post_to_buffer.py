...
def _upload_to_github(
    video_path: Path, reel_slug: str, dry_run: bool,
    image_path: Path | None = None, release_title: str | None = None,
) -> tuple[str | None, str | None]:
    """Upload MP4 (and optionally a cover image) to a GitHub release.

    Returns (video_url, image_url). image_url is None if no image was provided.
    """
    ...
    if result.returncode != 0:
        ...
        if "already exists" in result.stderr:
            print(f"  Release exists, uploading assets...")
            r2 = subprocess.run(
...
def main() -> None:
    ...
    # Step 1 — host the video (and cover image) on GitHub
    print("\n▸ Hosting video on GitHub releases...")
    src_dir = reel_dir / "_src"
    cover_image = next(
        (p for p in sorted(src_dir.glob("src_01.*")) if p.suffix in {".jpg", ".jpeg", ".png", ".webp"}),
        None,
    ) if src_dir.is_dir() else None
    if cover_image:
        print(f"  Cover image: {cover_image.name}")
    else:
        print("  No src_01 image found in _src/ — uploading video only")

    date_str = datetime.now().strftime("%Y-%m-%d")
    parts = [p for p in [artist, title, date_str] if p]
    release_title = " — ".join(parts) if parts else None

    video_url, _ = _upload_to_github(video_path, reel_slug, args.dry_run, cover_image, release_title)
    if not video_url:
        sys.exit(1)
